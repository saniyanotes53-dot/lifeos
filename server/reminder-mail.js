...
export const mailReady=()=>Boolean(process.env.RESEND_API_KEY&&process.env.EMAIL_FROM&&process.env.FIREBASE_SERVICE_ACCOUNT_JSON&&process.env.CRON_SECRET);
...
export async function adminToken(){
 ...
 const assertion=await new SignJWT({scope:'https://www.googleapis.com/auth/datastore https://www.googleapis.com/auth/firebase.messaging'}).setProtectedHeader({alg:'RS256'}).setIssuer(account.client_email).setAudience('https://oauth2.googleapis.com/token').setIssuedAt().setExpirationTime('5m').sign(await importPKCS8(account.private_key,'RS256'));
