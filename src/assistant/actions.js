...
export function validateActions(actions){
  ...
  return actions.map(a=>{
    if(!a||!actionTypes.includes(a.type)||!id(a.ref)||a.ref.length>128||refs.has(a.ref))throw new Error('The proposal contains an invalid action.');
    refs.add(a.ref);
...
export function actionDescription(a){
  ...
  if(a.type==='tag_transaction')return `${a.eventTag?'Tag':'Untag'} transaction: ${a.before.note||a.before.category} · ₹${a.before.amount}${a.eventTag?` → #${a.eventTag}`:''}`;
