...
export default function AuthExperience({t,scheme,setScheme}) {
 return <main className="auth-experience" data-scheme={scheme} style={{'--auth-accent':t.a1,'--auth-light':t.a2,'--auth-deep':t.a3,'--focus-color':t.a2}}>
  ...
  <header className="auth-masthead"><BrandLogo scheme={scheme}/>{scheme!=='blackhat'&&<div className="auth-palette" role="group" aria-label="Colour theme">{['blue','brown','peach'].map(c=><button key={c} aria-label={`${c} theme`} aria-pressed={scheme===c} onClick={()=>setScheme(c)} style={{'--swatch':{blue:'#7eb9ff',brown:'#bb854b',peach:'#ffb092'}[c]}}><span/>{c}</button>)}</div>}</header>
  <div className="auth-stage"><section className="auth-story"><span className="cinema-kicker">YOUR LIFE. A LITTLE LIGHTER.</span><h1>Space to <em>be you.</em></h1></section>
